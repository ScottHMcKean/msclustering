...
#'
#' @param ms_outlier microseismic data in a dataframe
...
run_gmm <- function(ms_outlier,
  ...
  ellipse_df <- data.frame("name" = character(), "x0" = numeric(),"y0" = numeric(),
                           ...
                           "plane_l3" = numeric(), "well_num" = numeric(),
                           "stage_num" = numeric(), "name" = character(),
                           "bool" = logical())
  ...
  ms_outlier$cluster = "0"

  #sort comp_df by start time for image indexing
  comp_sort <- arrange(comp_df, get(comp_start_col))
  comp_sort$row <- row.names(comp_sort)

  for (well_num in unique(ms_outlier$class_well)){
    ...
    well_ms <- subset(ms_outlier, class_well == well_num)

    print(paste("well",well_num))
    print(table(ms_outlier$class_stage))

    #for (stage_num in c(2)){
    for (stage_num in unique(well_ms$class_stage)){
      print(paste("well",well_num,"stage",stage_num))

      ## Set a prefix to label all outputs and allow multiple data analyses
      prefix <- paste('GMM_',well_num,'_',stage_num,sep="")
      order <- as.character(comp_sort %>% filter(well == well_num & stage == stage_num) %>% select(row))

      ## Subset to a single stage
      stage_ms <- well_ms[well_ms$class_stage == stage_num,]
      ...
      # CLUSTER HF DATA
      if (nrow(stage_hf_ms) > 4){

        ## Select attributes
        stage_hf_ms <- stage_hf_ms %>%
          mutate()
        ...
        ms_outlier[(ms_outlier$class_well == well_num) & (ms_outlier$class_stage == stage_num) &
                (ms_outlier$bool == 0),'cluster'] <- paste(well_num,
                                                    "_",stage_num,
                                                    "_HF",
        ...
        gg_cluster_plot(stage_ms[stage_ms$bool == 0,], comp_df, surv_df,
                        well = well_num, stage = stage_num, outlier = 0, order,
        ...
        for (i in 1:gmm_res$G){
          # name cluster
          name <- paste("W",well_num,"_S",stage_num,"_HF_C",i,sep="")
          ...
          obs <- data.frame("name" = name, "x0" = mean_i[1],"y0" = mean_i[2],
                            ...
                            "well_num" = well_num, "stage_num" = stage_num,
                            "cluster_num" = i)
      ...
      if (nrow(stage_out_ms) > 4){

        ## Select attributes
        stage_out_ms[,ms_time_col] <- as.numeric(stage_out_ms[,ms_time_col])
        ...
        ms_outlier[(ms_outlier$class_well == well_num) & (ms_outlier$class_stage == stage_num) &
                ms_outlier$bool == 1,'cluster'] <- paste(well_num,
                                                        "_",stage_num,
                                                        "_OUT",
                                                        as.character(gmm_res$classification),sep = "")
        ...
        gg_cluster_plot(stage_ms[stage_ms$bool == 1,], comp_df, surv_df,
                        well = well_num, stage = stage_num, outlier = 1, order,
                        title = title_bool, legend_bool = legend_bool)
        }
        ...
        for (i in 1:gmm_res$G){
          # name cluster
          name <- paste("W",well_num,"_S",stage_num,"_OUT_C",i,sep="")
  ...
  write.csv(ellipse_df,"xyzt_clusters.csv", row.names = FALSE)
  write.csv(cov_df,"xyzt_covariancedata.csv",row.names = FALSE)

  paste0('elapsed time: ', Sys.time() - start_time)

  write.csv(ms_outlier, 'ms_gmm_results.csv')
